...
type StateEntry = (CharInts, String, Trie)
...
readableEntry :: StateEntries -> String
readableEntry m = show $ map (\(i, ss) -> (map (\(_, s, _) -> (i, reverse s)) ss)) $ M.toList m
          
--foldr (++) [] $ map (\(i, ss) -> map (\s -> (i, reverse s)) ss) (M.toList m)
